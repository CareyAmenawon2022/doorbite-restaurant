...
function printReceipt(order, restaurantName) {
  ...
  const itemRows = order.items?.map(i =>
    `<tr>
      <td style="padding:5px 0;font-size:13px;">${i.name} <span style="color:#777;">×${i.quantity}</span></td>
      <td style="padding:5px 0;font-size:13px;text-align:right;font-weight:700;">₦${((i.price||0) * (i.quantity||1)).toLocaleString()}</td>
    </tr>`
  ).join('') || '';

  const feeRows = [
    ['Subtotal', `₦${(order.subtotal||0).toLocaleString()}`],
    ['Delivery fee', `₦${(order.deliveryFee||1000).toLocaleString()}`],
    ['Service fee (10%)', `₦${(order.serviceFee||0).toLocaleString()}`],
    ...(order.smallOrderFee > 0 ? [['Small order fee', `₦${order.smallOrderFee.toLocaleString()}`]] : []),
  ].map(([label, val]) =>
    `<tr><td style="font-size:12px;color:#666;padding:3px 0;">${label}</td><td style="font-size:12px;text-align:right;">${val}</td></tr>`
  ).join('');
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family:'Courier New',monospace; padding:24px 20px; max-width:320px; margin:0 auto; color:#111; }
    .center { text-align:center; }
    .dashed { border:none; border-top:1px dashed #aaa; margin:12px 0; }
    table { width:100%; border-collapse:collapse; }
    .total-row td { font-weight:900; font-size:16px; border-top:1px dashed #aaa; padding-top:10px; margin-top:4px; }
...
<body>
  ...
  <div class="center" style="margin-bottom:14px;">
    ...
    <div style="font-size:18px;font-weight:900;letter-spacing:1px;">${restaurantName || 'DoorBite'}</div>
    <div style="font-size:11px;color:#777;margin-top:3px;">Powered by DoorBite</div>
  ...
  <div style="margin-bottom:10px;">
    <div style="font-size:14px;font-weight:900;margin-bottom:6px;">ORDER #${order.orderCode}</div>
    <table>
      <tr><td style="font-size:12px;color:#666;width:80px;">Customer</td><td style="font-size:12px;font-weight:700;">${order.customer?.name || '—'}</td></tr>
      <tr><td style="font-size:12px;color:#666;">Phone</td><td style="font-size:12px;">${order.customer?.phone || '—'}</td></tr>
      <tr><td style="font-size:12px;color:#666;vertical-align:top;padding-top:2px;">Deliver to</td><td style="font-size:12px;line-height:1.4;">${order.deliveryAddress?.address || '—'}</td></tr>
    </table>
  </div>
  ...
  <!-- Items -->
  <div style="font-size:11px;font-weight:700;color:#777;letter-spacing:0.5px;margin-bottom:6px;">ITEMS</div>
  <table>
  ...
  <!-- Payment ref -->
  <div class="center" style="font-size:11px;color:#777;margin-bottom:14px;">
    ✓ Paid via Paystack<br/>
    ${order.paystackReference ? `Ref: ${order.paystackReference}` : ''}
  </div>

  <!-- Thank you -->
  <div class="center" style="font-size:16px;font-weight:900;margin-bottom:4px;">Thank you! 🎉</div>
  <div class="center" style="font-size:11px;color:#777;line-height:1.8;">
    Please keep this receipt for your records.<br/>
    Support: support@doorbite.ng
  </div>

  <!-- Print button -->
  <div class="center no-print" style="margin-top:24px;">
    <button onclick="window.print();"
      style="background:#FF6B2C;color:#fff;border:none;padding:11px 32px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;margin-right:10px;">
      🖨️ Print
    </button>
    <button onclick="window.close();"
      style="background:#f5f5f5;color:#333;border:1px solid #ddd;padding:11px 20px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;">
      Close
    </button>
  </div>
...
function LocationPicker({ value, onChange, label = 'Restaurant Location' }) {
  ...
  useEffect(() => {
    ...
    mapObjRef.current.panTo({ lat: value.lat, lng: value.lng });
    mapObjRef.current.setZoom(17);
    placeMarker(mapObjRef.current, { lat: value.lat, lng: value.lng });
  }, [value?.lat, value?.lng]);

  const placeMarker = (map, pos) => {
    if (markerRef.current) { markerRef.current.setPosition(pos); }
    else { markerRef.current = new window.google.maps.Marker({ position: pos, map, draggable:true, animation: window.google.maps.Animation.DROP }); attachMarkerDrag(markerRef.current); }
  };
  ...
  const reverseGeocode = (lat, lng) => {
    new window.google.maps.Geocoder().geocode({ location: { lat, lng } }, (results, status) => {
      ...
      setQuery(addr); onChange({ lat, lng, address: addr });
    });
  };
  const handleSearch = (e) => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      setSearching(true);
      svcRef.current.getPlacePredictions({ input: q, componentRestrictions: { country: 'ng' }, sessionToken: tokenRef.current, location: new window.google.maps.LatLng(6.3350, 5.6037), radius: 60000 },
        (preds, status) => { setSearching(false); setSuggestions(status === 'OK' && preds ? preds : []); });
    }, 350);
  };
  const pickSuggestion = (pred) => {
    ...
    new window.google.maps.Geocoder().geocode({ placeId: pred.place_id }, (results, status) => {
      ...
      if (status === 'OK' && results[0]) {
        const loc = results[0].geometry.location; const lat = loc.lat(); const lng = loc.lng(); const addr = results[0].formatted_address;
        setQuery(addr); tokenRef.current = new window.google.maps.places.AutocompleteSessionToken();
        if (mapObjRef.current) { mapObjRef.current.panTo({ lat, lng }); mapObjRef.current.setZoom(17); placeMarker(mapObjRef.current, { lat, lng }); }
  ...
  const applyManual = () => {
    ...
    if (mapObjRef.current) { mapObjRef.current.panTo({ lat, lng }); mapObjRef.current.setZoom(17); placeMarker(mapObjRef.current, { lat, lng }); }
    reverseGeocode(lat, lng); setShowManual(false);
  };
    ...
    <div>
      ...
      <div style={{ position:'relative', marginBottom:8 }}>
        <div style={{ display:'flex', gap:8 }}>
          <div style={{ position:'relative', flex:1 }}>
            ...
            {value?.lat && !searching && <button onClick={clear} style={{ position:'absolute', right:10, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:C.gray, fontSize:17 }}>✕</button>}
          </div>
          ...
          <div style={{ position:'absolute', top:'100%', left:0, right:48, background:'#fff', border:`1px solid ${C.border}`, borderRadius:10, boxShadow:'0 8px 24px rgba(0,0,0,0.12)', zIndex:400, maxHeight:220, overflowY:'auto', marginTop:4 }}>
            {suggestions.map((p, i) => (
              <div key={p.place_id} onClick={() => pickSuggestion(p)} style={{ padding:'11px 14px', cursor:'pointer', borderBottom:i<suggestions.length-1?`1px solid ${C.border}`:'none', fontSize:13, lineHeight:1.4 }} onMouseEnter={e => e.currentTarget.style.background='#FFF7ED'} onMouseLeave={e => e.currentTarget.style.background='#fff'}>
                <span style={{ marginRight:8 }}>📍</span><strong>{p.structured_formatting?.main_text}</strong>{p.structured_formatting?.secondary_text && <span style={{ color:C.gray }}> · {p.structured_formatting.secondary_text}</span>}
              </div>
            ))}
      ...
      {showManual && (
        <div style={{ background:'#FFF7ED', borderRadius:10, padding:14, marginBottom:8, border:`1px solid #FED7AA` }}>
          <div style={{ fontSize:12, color:'#92400E', fontWeight:700, marginBottom:10 }}>📐 Enter coordinates manually <span style={{ fontWeight:400 }}>— from Google Maps: right-click → copy numbers at top</span></div>
          <div style={{ display:'flex', gap:10, alignItems:'flex-end' }}>
            <div style={{ flex:1 }}><label style={{ fontSize:11, fontWeight:700, color:'#92400E', display:'block', marginBottom:4 }}>LATITUDE</label><input style={{ ...inp, borderColor:manualLat&&!isNaN(parseFloat(manualLat))?C.success:C.border, fontSize:13 }} placeholder="e.g. 6.33450" value={manualLat} onChange={e=>setManualLat(e.target.value)} /></div>
            <div style={{ flex:1 }}><label style={{ fontSize:11, fontWeight:700, color:'#92400E', display:'block', marginBottom:4 }}>LONGITUDE</label><input style={{ ...inp, borderColor:manualLng&&!isNaN(parseFloat(manualLng))?C.success:C.border, fontSize:13 }} placeholder="e.g. 5.62710" value={manualLng} onChange={e=>setManualLng(e.target.value)} /></div>
      ...
      <div style={{ position:'relative', borderRadius:12, overflow:'hidden', border:`2px solid ${value?.lat ? C.success : C.border}` }}>
        ...
        {!gmapsReady && <div style={{ position:'absolute', inset:0, background:'#f5f5f5', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:10 }}><div style={{ fontSize:32 }}>🗺️</div><div style={{ color:C.gray, fontSize:13, fontWeight:600 }}>Loading Google Maps...</div></div>}
...
function Register({ onBack }) {
  ...
  const handle = async e => {
    e.preventDefault();
    if (!form.ownerName||!form.email||!form.password||!form.phone||!form.restaurantName||!form.cuisineType||!form.address) return setError('Please fill in all required fields');
    setLoading(true); setError('');
    try { await axios.post(`${API}/auth/register-restaurant`, { ...form, location: location ? { lat:location.lat, lng:location.lng } : undefined }); setSuccess(true); }
      ...
      <div style={{flex:1,overflowY:'auto',display:'flex',alignItems:'flex-start',justifyContent:'center',padding:40}}>
        <form onSubmit={handle} style={{background:'#fff',borderRadius:24,padding:36,width:'100%',maxWidth:580,boxShadow:'0 4px 24px rgba(0,0,0,0.08)'}}>
          ...
          <div style={{background:'#FFF7ED',borderRadius:12,padding:16,marginBottom:20,border:`1px solid #FED7AA`}}>
            ...
            <p style={{color:'#92400E',fontSize:13,marginBottom:14}}>This helps riders navigate to your restaurant accurately. Search your address or enter coordinates.</p>
            <LocationPicker value={location} onChange={setLocation} label="Search your restaurant address" />
            {!location && <div style={{background:'#FEF3C7',borderRadius:8,padding:10,marginTop:12,border:`1px solid #FDE68A`}}><div style={{fontSize:12,color:'#92400E',fontWeight:600}}>⚠️ Location not set yet</div><div style={{fontSize:11,color:'#B45309',marginTop:2}}>You can also set this later from your Settings dashboard.</div></div>}
          </div>
    ...
    <div style={{display:'flex',height:'100vh'}}>
      ...
      <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:C.bg}}>
        <form onSubmit={handle} style={{background:'#fff',borderRadius:24,padding:36,width:420,boxShadow:'0 4px 24px rgba(0,0,0,0.08)'}}>
          ...
          <input style={{...inp,marginBottom:24}} type="password" placeholder="••••••••" value={password} onChange={e=>setPassword(e.target.value)} />
      ...
      {liveOrders.length===0?(<div style={card}><p style={{color:C.gray,textAlign:'center',padding:20}}>No active orders right now 🎉</p></div>):liveOrders.map(order=>(
        <div key={order._id} style={{...card,borderLeft:`4px solid ${C.primary}`}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <div><div style={{color:C.gray,fontSize:12}}>#{order.orderCode} · {new Date(order.createdAt).toLocaleTimeString()}</div><div style={{fontWeight:700,fontSize:16}}>{order.customer?.name}</div><div style={{color:C.gray,fontSize:13}}>{order.items?.map(i=>`${i.name} ×${i.quantity}`).join(' · ')}</div></div>
            <div style={{textAlign:'right'}}><div style={{fontSize:20,fontWeight:800}}>₦{order.total?.toLocaleString()}</div><div style={{fontSize:12,color:C.success,fontWeight:600}}>You get: ₦{Math.round((order.subtotal||0)*0.90).toLocaleString()}</div></div>
          </div>
          ...
          {/* ── PRINT RECEIPT — show once confirmed ── */}
          {order.status !== 'pending' && (
            <button
              onClick={() => printReceipt(order, restaurant?.name)}
              style={{ ...btn('#fff'), color:C.gray, border:`1px solid ${C.border}`, marginTop:10, width:'100%', fontSize:12 }}
            >
              🖨️ Print Receipt
            </button>
          )}
  ...
  return (
    <div style={{padding:28,overflowY:'auto',flex:1}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap'}}>
        {Object.keys(TABS).map(t=>{ const count=t==='pending'?orders.filter(o=>o.status==='pending').length:0; return <button key={t} onClick={()=>setTab(t)} style={{...btn(tab===t?C.primary:'#fff'),color:tab===t?'#fff':C.gray,border:`1px solid ${C.border}`,borderRadius:20,textTransform:'capitalize'}}>{t}{count>0?` (${count})`:''}</button>; })}
      </div>
      {filtered.length===0?<p style={{color:C.gray,textAlign:'center',marginTop:60}}>No {tab} orders</p>:filtered.map(order=>(
        <div key={order._id} style={{...card,borderLeft:order.status==='pending'?`4px solid ${C.primary}`:'none'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
            <div>
              <span style={{color:C.gray,fontSize:12}}>#{order.orderCode} · {order.customer?.name} · {order.customer?.phone}</span>
              <div style={{fontWeight:700,fontSize:15,marginTop:2}}>{order.items?.map(i=>`${i.name} ×${i.quantity}`).join(', ')}</div>
            </div>
            <div style={{textAlign:'right'}}>
              <div style={{fontSize:20,fontWeight:800}}>₦{order.total?.toLocaleString()}</div>
              <div style={{fontSize:12,color:C.success,fontWeight:600}}>Your cut: ₦{Math.round((order.subtotal||0)*0.90).toLocaleString()}</div>
              <span style={{background:(SC[order.status]||'#ccc')+'22',color:SC[order.status]||'#ccc',padding:'3px 10px',borderRadius:20,fontSize:12,fontWeight:700}}>{order.status}</span>
            </div>
          </div>
          {/* Delivery address */}
          {order.deliveryAddress?.address && (
            <div style={{fontSize:12,color:C.gray,marginBottom:8}}>📍 {order.deliveryAddress.address}</div>
          )}
          {/* Action buttons */}
          ...
          {/* ── PRINT RECEIPT — show for all statuses except pending/rejected/cancelled ── */}
          {!['pending','rejected','cancelled'].includes(order.status) && (
            <button
              onClick={() => printReceipt(order, restaurant?.name)}
              style={{ ...btn('#fff'), color:'#555', border:`1px solid ${C.border}`, marginTop:10, width:'100%', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}
            >
              🖨️ Print Receipt for Customer
            </button>
          )}
        </div>
      ))}
...
function Wallet() {
  ...
  useEffect(()=>{ refresh(); },[]);
  const requestWithdrawal=async()=>{ const amount=Number(wAmount); if(!amount||amount<100) return alert('Minimum withdrawal is ₦100'); if(amount>(data?.walletBalance||0)) return alert('Amount exceeds your wallet balance'); if(!restaurant?.bankDetails?.accountNumber) return alert('Please save your bank details in Settings first.'); setWLoading(true); try{ await api.post('/withdrawals',{amount}); alert('✅ Withdrawal request submitted!'); setWAmount(''); refresh(); }catch(err){ alert(err.response?.data?.message||err.message); }finally{ setWLoading(false); } };
...
function Settings() {
  ...
  const save=async()=>{
    setSaving(true);
    try{ const payload={...form}; if(location?.lat) payload.location={lat:location.lat,lng:location.lng}; const {data}=await api.patch('/restaurants/me',payload); setRestaurant(data); alert('Settings saved!'); }
    ...
    <div style={{padding:28,overflowY:'auto',flex:1}}>
      ...
      <div style={{...card,marginBottom:16}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:20}}>
          <div style={{width:120,height:90,borderRadius:14,overflow:'hidden',background:'#f5f5f5',border:`2px dashed ${C.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:36,flexShrink:0}}>{form.logo?<img src={form.logo} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}} />:'🏪'}</div>
          <div><label style={{...btn(C.primary),display:'inline-block',cursor:'pointer',padding:'10px 20px',fontSize:14}}>{uploadingLogo?'⏳ Uploading...':'📷 Upload Photo'}<input type="file" accept="image/*" onChange={handleLogoUpload} style={{display:'none'}} disabled={uploadingLogo} /></label><p style={{color:C.gray,fontSize:12,marginTop:8}}>JPG or PNG, at least 400×300px.</p>{form.logo&&<p style={{color:C.success,fontSize:12,marginTop:4,fontWeight:600}}>✓ Photo uploaded</p>}</div>
        </div>
      </div>
      <div style={{...card,marginBottom:16}}>
      ...
      <div style={{...card,marginBottom:16,borderTop:`3px solid ${C.primary}`}}>
        ...
        <p style={{color:C.gray,fontSize:13,marginBottom:16}}>Used by riders to navigate to your restaurant. Search your address to pin the exact location — a map preview will appear to confirm.</p>
        <LocationPicker value={location} onChange={setLocation} label="Search and pin your restaurant location" />
        <div style={{marginTop:12,fontSize:12,color:C.gray,background:C.bg,borderRadius:8,padding:10}}>💡 After setting your location, click <strong>Save Changes</strong> at the top to apply it.</div>
      </div>
